...
SkyManager::SkyManagerData(AstroTime& astroTimeRef, Location& locationRef, Sun& sunRef, Moon& moonRef){
...
void SkyManger::update(boolean updatePlanets){
  ...
  // 1.0 / 3526000.0
  #define MAOL_CONST1 0.0000002.836074872376630743051616562677254679523539421440
  // 1.0 / 863310000.0
  ...
  double omega = moon->getLongitudeOfTheAscendingNodeOfOrbit() * DEG_2_RAD;
  double omegaTimes2 = omega * 2.0;
  double sunsMeanLongitudeInRadsTimes2 = sun->meanLongitude * DEG_2_RAD_TIMES_2;
  double moonsMeanLongitudeInRadsTimes2 = moon->meanLongitude * DEG_2_RAD_TIMES_2;
  ...
  astroTime.updateLocalApparentSiderealTime(location->longitude);

  //And back to our sun again
  double sunsMeanAnomolyInRads = sun->meanAnomoly * DEG_2_RAD;
