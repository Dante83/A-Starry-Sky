  ...
  fragmentShader: function(mieG, textureWidth, textureHeight, packingWidth, packingHeight, atmosphereFunctions, sunCode = false, moonCode = false, meteringCode = false, auroraEnabled = false){
    ...
    '#if($auroraEnabled)',
      ...
      'vec3 auroraRayMarchPass(vec3 rayStartPosition, vec3 rayDirection, float starAndSkyExposureReduction){',
        'float uvScaling = 4.0;',
        'float rayInterceptStartTime = min(interceptPlaneSurface(rayStartPosition, rayDirection, RADIUS_OF_AURORA_BOTTOM, 0.0), 2500.0);',
        'float rayInterceptEndTime = min(interceptPlaneSurface(rayStartPosition, rayDirection, RADIUS_OF_AURORA_TOP, 0.0), 3300.0);',
    ...
    'float simplex3dFractal(vec3 m, vec2 cloudVelocity, float cloudDensity, float heightPercentage) {',
      ...
      '//simplexFractal *= max(2.0 - 2.0 * heightPercentage, 1.0);',
    ...
    'float henyayGreenstein(float cosOfVAndL,float g){',
      'return (1.0 / PI_TIMES_FOUR) * (1.0 - g * g) / pow(1.0 + g * g - 2.0 * g * cosOfVAndL, 3.0 / 2.0);',
    '}',
    ...
    'vec4 cloudRayMarcher(float cloudCoverage, vec2 cloudVelocity, vec3 rayStartPosition, vec3 rayDirection, float starAndSkyExposureReduction, vec3 sunLightDirection, vec3 directSolarLightColor){',
      ...
      'float numberOfInnerCloudMarchSteps = 32.0;',
      ...
      'if(rayInterceptStartTime > 0.0){',
        ...
        'float heightPercentage = (lastPosition.y - cloudStartHeight) / (cloudEndHeight - cloudStartHeight);',
        'float cloudDensity0 = simplex3dFractal(lastPosition, cloudVelocity, cloudCoverage, heightPercentage);',
        ...
        'for(float i = 0.0; i < numberOfCloudMarchSteps; i++){',
          ...
          'vec3 currentPosition = lastPosition + rayDirection * (rayDeltaT * (1.0 + blueNoise));',
          'float d = distance(currentPosition, lastPosition);',
          'heightPercentage = (currentPosition.y - cloudStartHeight) / (cloudEndHeight - cloudStartHeight);',
          ...
          'transmittance = exp(-0.1 * cloudDensity);',
          ...
          'if(cloudDensity > 0.00){',
            ...
            'if(sunLightDirection.y > 0.0){',
              'innerInterceptEndTime = interceptPlaneSurface(innerLastPosition, sunLightDirection, rayStartPosition.y + cloudEndHeight, 1000.0);',
            '}',
            'else{',
              'innerInterceptEndTime = interceptPlaneSurface(innerLastPosition, sunLightDirection, rayStartPosition.y + cloudStartHeight, 1000.0);',
            '}',
            ...
            'for(float j = 0.0; j < numberOfInnerCloudMarchSteps; j++){',
              ...
              'vec3 innerCurrentPosition = innerLastPosition + sunLightDirection * (innerDeltaT * (1.0 + innerBlueNoise));',
              'float dInnerRay = distance(innerCurrentPosition, innerLastPosition);',

              '//Calculate our transmittance to this point',
              'heightPercentage = (innerCurrentPosition.y - cloudStartHeight) / (cloudEndHeight - cloudStartHeight);',
              ...
              'innerTransmittance = exp(-0.1 * innerCloudDensity);',
          ...
          'luminance += 0.0006 * innerTransmittance * d * transmittance * henyayGreenstein(dot(rayDirection, sunLightDirection), g);',
        ...
        '}',
      '}',
      'luminance += 0.02 * vec3(0.95, 0.97, 1.0) * (1.0 - transmittance);',
      'return vec4(luminance * directSolarLightColor * max(sunHorizonFade, moonHorizonFade), 1.0 - transmittance); //Linear multiplier for artistic control',
    '}',
    ...
    'void main(){',
      ...
      'vec3 dominantLightSourceColor = scatteringMoonIntensity * moonLightColor * moonPosition.y;',
      ...
      'vec4 cloudLighting = cloudRayMarcher(cloudCoverage, cloudVelocity, vec3(0.0, RADIUS_OF_EARTH * 1000.0, 0.0), sphericalPosition, 0.0, dominantLightSourcePosition, dominantLightSourceColor * transmittanceOfPrimaryLightSource);',
