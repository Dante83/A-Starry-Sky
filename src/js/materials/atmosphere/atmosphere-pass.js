StarrySky.Materials.Atmosphere.atmosphereShader = {
  uniforms: function(isSunShader = false, isMoonShader = false, isMeteringShader = false, auroraEnabled = false){
    let uniforms = {
      uTime: {value: 0.0},
      ...
      ambientLightPY: {value: new THREE.Vector3()},
  ...
  fragmentShader: function(mieG, textureWidth, textureHeight, packingWidth, packingHeight, atmosphereFunctions, sunCode = false, moonCode = false, meteringCode = false, auroraEnabled = false){
    let originalGLSL = [
    ...
    '#if($auroraEnabled)',
      ...
      'vec3 auroraRayMarchPass(vec3 rayStartPosition, vec3 rayDirection, float starAndSkyExposureReduction){',
        'float uvScaling = 4.0;',
        'float rayInterceptStartTime = interceptPlaneSurface(rayStartPosition, rayDirection, RADIUS_OF_AURORA_BOTTOM, 400000.0);',
        'float rayInterceptEndTime = interceptPlaneSurface(rayStartPosition, rayDirection, RADIUS_OF_AURORA_TOP, 400000.0);',
    ...
    'float simplex3dFractal(vec3 m, vec2 cloudVelocity, float cloudDensity, float heightPercentage) {',
      ...
      'offsetM = offsetM * 0.001 * vec3(0.15, 0.30, 0.15);',
      '// float simplexFractal = 0.5333333*simplex3d(offsetM*rot1) + 0.2666667*simplex3d(2.0*offsetM*rot2)',
      '// + 0.1333333*simplex3d(4.0*offsetM*rot3) + 0.0666667*simplex3d(8.0*offsetM);',
      'float simplexFractal = 0.5*simplex3d(offsetM*rot1) + pow(0.5, 2.0)*simplex3d(2.0*offsetM*rot2)',
      '+ pow(0.5, 3.0)*simplex3d(4.0*offsetM*rot3) + pow(0.5, 4.0)*simplex3d(8.0*offsetM)',
      '+ pow(0.5, 5.0)*simplex3d(16.0 * offsetM*rot1) + pow(0.5, 6.0)*simplex3d(32.0*offsetM*rot2)',
      '+ pow(0.5, 7.0)*simplex3d(64.0 * offsetM*rot1) + pow(0.5, 8.0)*simplex3d(128.0*offsetM*rot2);',
      'simplexFractal = clamp(0.5 * simplexFractal + 0.5, 0.0, 1.0);',
      'simplexFractal = min(cloudDensity - simplexFractal, 0.0) / (cloudDensity - 1.0);',
      'vec3 worleyNoise = texture(cloudLUTs, offsetM * 4.0).rgb;',
      'float cloudNoise = worleyNoise.r * 0.625 + worleyNoise.g * 0.125 + worleyNoise.b * 0.25;',
      'float simplexFractalWithBillows = mix(max(simplexFractal - 0.5 * (1.0 - cloudNoise), 0.0), simplexFractal, simplexFractal);',
      'simplexFractal = mix(simplexFractalWithBillows, simplexFractal, linearGradient(0.0, 0.1, heightPercentage));',
      'simplexFractal *= linearGradient(0.0, 0.05, heightPercentage);',
      'simplexFractal *= linearGradient(1.0, 0.9, heightPercentage);',
      'return simplexFractal;',
    '}',

    'float henyayGreenstein(float cosOfVAndL,float g){',
      'return (1.0 / PI_TIMES_FOUR) * (1.0 - g * g) / pow(1.0 + g * g - 2.0 * g * cosOfVAndL, 1.5);',
    '}',

    'vec4 cloudRayMarcher(float cloudCoverage, vec2 cloudVelocity, vec3 rayStartPosition, vec3 rayDirection, float starAndSkyExposureReduction, vec3 dominantLightDirection, vec3 dominantLightSourceColor, vec3 atmosphericFog){',
      '//This is in meters',
      'float cloudStartHeight = 1000.0;',
      'float cloudEndHeight = 2500.0;',
      ...
      'float numberOfCloudMarchSteps = 64.0;',
      '//float numberOfInnerCloudMarchSteps = 0.0;',
      'float rayInterceptStartTime = interceptPlaneSurface(rayStartPosition, rayDirection, rayStartPosition.y + cloudStartHeight, 40000.0);',
      'float rayInterceptEndTime = interceptPlaneSurface(rayStartPosition, rayDirection, rayStartPosition.y + cloudEndHeight, 40000.0);',
      ...
      'if(rayInterceptStartTime > 0.0){',
        ...
        'for(float i = 0.0; i < numberOfCloudMarchSteps; i++){',
          '//Determine the position of our raymarcher in the sky',
          'float blueNoise = texture(blueNoiseTexture, vec2(lastPosition.x + uTime, lastPosition.z + uTime) / 128.0).r * 2.0 - 1.0;',
          'vec3 currentPosition = lastPosition + rayDirection * (rayDeltaT * (1.0 + 0.25 * blueNoise));',
          'float d = distance(currentPosition, lastPosition);',
          ...
          'rayTransmittance = exp(-0.1 * cloudDensity);',

          '//Determine the luminance',
          'float innerTransmittance = 1.0;',
          "//NOTE: Turning this off because it's too hard on the GPU",
          '// if(cloudDensity > 0.00){',
          '//   vec3 innerLastPosition = currentPosition;',
          '//   float innerInterceptEndTime;',
          '//   if(dominantLightDirection.y > 0.0){',
          '//     innerInterceptEndTime = interceptPlaneSurface(innerLastPosition, dominantLightDirection, rayStartPosition.y + cloudEndHeight, 250.0);',
          '//   }',
          '//   else{',
          '//     innerInterceptEndTime = interceptPlaneSurface(innerLastPosition, dominantLightDirection, rayStartPosition.y + cloudStartHeight, 250.0);',
          '//   }',
          '//   innerInterceptEndTime = min(innerInterceptEndTime, 2000.0);',
          '//   float innerDeltaT = numberOfInnerCloudMarchSteps != 0.0 ? innerInterceptEndTime / numberOfInnerCloudMarchSteps : 1.0;',
          '//   float innerCloudDensity0 = cloudDensityf;',
          '//   float innerCloudDensity = 0.0;',
          '//',
          '//   //Inner ray path',
          '//   for(float j = 0.0; j < numberOfInnerCloudMarchSteps; j++){',
          '//     float innerBlueNoise = texture(blueNoiseTexture, vec2(innerLastPosition.x + uTime, innerLastPosition.z + uTime) / 128.0).r * 2.0 - 1.0;',
          '//     vec3 innerCurrentPosition = innerLastPosition + dominantLightDirection * (innerDeltaT * (1.0 + innerBlueNoise));',
          '//     float dInnerRay = distance(innerCurrentPosition, innerLastPosition);',
          '//',
          '//     //Calculate our transmittance to this point',
          '//     heightPercentage = (innerCurrentPosition.y - globalCloudStartHeight) / (globalCloudEndHeight - globalCloudStartHeight);',
          '//     float innerCloudDensityf = simplex3dFractal(innerCurrentPosition, cloudVelocity, cloudCoverage, heightPercentage);',
          '//     innerCloudDensity += 0.5 * (innerCloudDensity0 + innerCloudDensityf) * dInnerRay;',
          '//     innerTransmittance = exp(-0.08 * innerCloudDensity);',
          '//',
          '//     innerLastPosition = innerCurrentPosition;',
          '//     innerCloudDensity0 = innerCloudDensityf;',
          '//   }',
          '// }',
          '//Update our luminance',
          'float lightSourceHeight = RADIUS_OF_EARTH + ((currentPosition.y / 1000.0) - RADIUS_OF_EARTH);',
          ...
          'float scatteringToRayPoint = henyayGreenstein(dot(dominantLightDirection, -dominantLightDirection), g);',
          'float scatteringToCamera = henyayGreenstein(dot(rayDirection, -dominantLightDirection), g);',
          'luminance += 0.15 * dominantLightSourceColor * dominantLightSourceAtmosphericTransmittance * innerTransmittance * d * rayTransmittance * scatteringToRayPoint * scatteringToCamera;',

          '//Update previous values',
          'cloudDensity0 = cloudDensityf;',
          'lastPosition = currentPosition;',
          'if(cloudDensity > 0.2 && !hasFirstContact){',
            'firstContactPosition = lastPosition;',
            'hasFirstContact = true;',
          '}',
        '}',
      '}',
      '#if(!$isMeteringPass)',
        'luminance += 0.05 * ambientLightPY * sqrt(dot(dominantLightSourceColor, dominantLightSourceColor)) * (1.0 - rayTransmittance);',
      '#endif',
      'if(hasFirstContact){',
    ...
    'void main(){',
      ...
      '#if(!$isMeteringPass)',
        ...
        'vec4 cloudLighting = cloudRayMarcher(1.0 - cloudCoverage, cloudVelocity, vec3(0.0, RADIUS_OF_EARTH * 1000.0, 0.0), sphericalPosition, 0.0, dominantLightSourcePosition, dominantLightSourceColor, solarAtmosphericPass + lunarAtmosphericPass);',
      '#endif',
