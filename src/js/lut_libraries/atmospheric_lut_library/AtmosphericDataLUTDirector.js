StarrySky.LUTlibraries.AtmosphericDataLUTDirector = class AtmosphericDataLUTDirector{
  constructor(scene, renderer, width, height, depth){
    ...
    this.mrtRenderTarget = new StarrySky.LUTlibraries.AtmosphericDataMRT(width, height, 2);
  ...
  this.render(material){
    let meiDataArrayBuffer = new ArrayBuffer(this.SCATTERING_DATA_SIZE);
    let meiData = new Float32Array(meiDataArrayBuffer);
    let rayleighDataArrayBuffer = new ArrayBuffer(this.SCATTERING_DATA_SIZE);
    let rayleighData = new Float32Array(rayleighDataArrayBuffer);
