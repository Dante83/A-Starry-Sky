...
var skyShaderMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: [
    ...
    'vec3 lightBlending(vec3 starColor, vec3 inColor, vec3 FexPixel){',
      '//The magnitude of the stars is dimmed according to the current brightness',
      '//but we now wish to keep the sky color the same',
      '//Note: Leaving our pow(2.2 behind to switch to square based light intensity addition)',
      '//as it probably works just as well and is likely faster',
      'return clamp(sqrt(starColor * starColor * FexPixel + inColor * inColor), 0.0, 1.0);',
    '}',
    ...
    '//STARS',
    '//',
    'vec4 drawStarLayer(float azimuthOfPixel, float altitudeOfPixel, vec3 skyColor){',
      ...
      'vec4 returnColor = vec4(skyColor, 1.0);',
    ...
    'void main(){',
      ...
      'vec3 sunColor = drawSunLayer(skyColor, FexPixel, cosTheta.x).rgb;',
